...
CREATE OR REPLACE FUNCTION public.admin_list_firmalar_v2(p_page integer DEFAULT 1, p_per_page integer DEFAULT 25, p_search text DEFAULT NULL::text, p_filter_turu text DEFAULT NULL::text, p_filter_tipi text DEFAULT NULL::text, p_filter_il text DEFAULT NULL::text, p_filter_ilce text DEFAULT NULL::text, p_filter_durum text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  WITH filtered_firmalar AS (
    SELECT f.id, f.firma_unvani, f.logo_url, f.created_at, f.updated_at,
           ...
           f.firma_olcegi_id, f.kurulus_tarihi, f.web_sitesi,
           f.firma_iletisim_numarasi, f.firma_iletisim_email,
           f.kapak_fotografi_url, f.firma_hakkinda
    FROM firmalar f
  ...
  enriched AS (
    SELECT
      ...
      -- Ağırlıklı profil doluluk oranı
      (
        (
          -- Genel bilgiler: her alan 2 puan (max 20)
          CASE WHEN pg.firma_olcegi_id IS NOT NULL THEN 2 ELSE 0 END +
          CASE WHEN pg.kurulus_tarihi IS NOT NULL AND pg.kurulus_tarihi != '' THEN 2 ELSE 0 END +
          CASE WHEN pg.kurulus_il_id IS NOT NULL THEN 2 ELSE 0 END +
          CASE WHEN pg.kurulus_ilce_id IS NOT NULL THEN 2 ELSE 0 END +
          CASE WHEN pg.web_sitesi IS NOT NULL AND pg.web_sitesi != '' THEN 2 ELSE 0 END +
          CASE WHEN pg.firma_iletisim_numarasi IS NOT NULL AND pg.firma_iletisim_numarasi != '' THEN 2 ELSE 0 END +
          CASE WHEN pg.firma_iletisim_email IS NOT NULL AND pg.firma_iletisim_email != '' THEN 2 ELSE 0 END +
          CASE WHEN pg.logo_url IS NOT NULL AND pg.logo_url != '' THEN 2 ELSE 0 END +
          CASE WHEN pg.kapak_fotografi_url IS NOT NULL AND pg.kapak_fotografi_url != '' THEN 2 ELSE 0 END +
          CASE WHEN pg.firma_hakkinda IS NOT NULL AND pg.firma_hakkinda != '' THEN 2 ELSE 0 END +
          -- Sekme verileri (ağırlıklı)
          CASE WHEN EXISTS (SELECT 1 FROM firma_urun_hizmet_secimler x WHERE x.firma_id = pg.id) THEN 6 ELSE 0 END +
          CASE WHEN EXISTS (SELECT 1 FROM firma_uretim_satis x WHERE x.firma_id = pg.id) THEN 7 ELSE 0 END +
          CASE WHEN EXISTS (SELECT 1 FROM firma_galeri x WHERE x.firma_id = pg.id) THEN 3 ELSE 0 END +
          CASE WHEN EXISTS (SELECT 1 FROM firma_sertifikalar x WHERE x.firma_id = pg.id) THEN 2 ELSE 0 END +
          CASE WHEN EXISTS (SELECT 1 FROM firma_referanslar x WHERE x.firma_id = pg.id) THEN 2 ELSE 0 END +
          CASE WHEN EXISTS (SELECT 1 FROM firma_tesisler x WHERE x.firma_id = pg.id) THEN 1 ELSE 0 END +
          CASE WHEN EXISTS (SELECT 1 FROM firma_belgeler x WHERE x.firma_id = pg.id) THEN 1 ELSE 0 END +
          CASE WHEN EXISTS (SELECT 1 FROM firma_turleri ftt WHERE ftt.id = pg.firma_turu_id AND ftt.name IN ('Hazır Giyim Üreticisi', 'Fason Atölye'))
               AND EXISTS (SELECT 1 FROM firma_makineler x WHERE x.firma_id = pg.id) THEN 1 ELSE 0 END
        ) * 100
      ) / CASE
            WHEN EXISTS (SELECT 1 FROM firma_turleri ftt WHERE ftt.id = pg.firma_turu_id AND ftt.name IN ('Hazır Giyim Üreticisi', 'Fason Atölye'))
            THEN 43
            ELSE 42
          END as profil_doluluk,
