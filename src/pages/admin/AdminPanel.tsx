...
export default function AdminPanel() {
  ...
  const panelData = {
    firma: {
      toplam: Number(data?.firma?.total ?? 0),
      onay_bekleyen: Number(data?.firma?.pending ?? 0),
    ...
    ihale: {
      toplam: Number(data?.ihale?.total ?? 0),
      aktif: Number(data?.ihale?.active ?? 0),
      tamamlanan: Number(data?.ihale?.completed ?? 0),
      iptal: Number(data?.ihale?.cancelled ?? 0),
      onay_bekleyen: Number(data?.ihale?.pendingApproval ?? 0),
      reddedilen: Number(data?.ihale?.rejected ?? 0),
      taslak: Number(data?.ihale?.draft ?? 0),
      urunKatDist: Array.isArray(data?.ihale?.urunKategoriDagilimi) ? data.ihale.urunKategoriDagilimi : [],
      hizmetKatDist: Array.isArray(data?.ihale?.hizmetKategoriDagilimi) ? data.ihale.hizmetKategoriDagilimi : [],
    },
    urun: {
      toplam: Number(data?.urun?.total ?? 0),
      aktif: Number(data?.urun?.active ?? 0),
      pasif: Number(data?.urun?.pasif ?? 0),
      onay_bekleyen: Number(data?.urun?.pending ?? 0),
      reddedilen: Number(data?.urun?.rejected ?? 0),
      taslak: Number(data?.urun?.draft ?? 0),
      katDist: Array.isArray(data?.urun?.katDist) ? data.urun.katDist : [],
      turDist: Array.isArray(data?.urun?.turDist) ? data.urun.turDist : [],
    },
    paket: {
      paketDist: Array.isArray(data?.paket?.dagilim) ? data.paket.dagilim : [],
    },
    destek: {
      toplam: Number(data?.destek?.total ?? 0),
      cozulen: Number(data?.destek?.open ?? 0),
      incelenen: Number(data?.destek?.inProgress ?? 0),
    },
    sikayet: {
      toplam: Number(data?.sikayet?.total ?? 0),
      ...
      beklemede: Number(data?.sikayet?.pending ?? 0),
