...
import { Check, Loader2, Search, Trash2, ClipboardList } from "lucide-react";
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <Select value={filterDurum} onValueChange={setFilterDurum}>
              <SelectTrigger className="w-32 h-9 text-xs" style={s.input}><SelectValue /></SelectTrigger>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {yapilacak.length > 0 && (
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-wider mb-3" style={s.muted}>Yapılacak ({yapilacak.length})</p>
                <div className="space-y-4">
                  {groupByDate(yapilacak).map(([date, items]) => (
                    <div key={date}>
                      <p className="text-[10px] font-medium mb-1.5 px-1" style={s.muted}>{date}</p>
                      <div className="space-y-1.5">
                        {items.map(a => <AksiyonCard key={a.id} aksiyon={a} onToggle={toggleDurum} onDelete={deleteAksiyon} />)}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {yapildi.length > 0 && (
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-wider mb-3" style={s.muted}>Tamamlanan ({yapildi.length})</p>
                <div className="space-y-1.5">
                  {yapildi.map(a => <AksiyonCard key={a.id} aksiyon={a} onToggle={toggleDurum} onDelete={deleteAksiyon} />)}
                </div>
              </div>
            )}
          </div>
...
function AksiyonCard({ aksiyon, onToggle, onDelete }: { aksiyon: Aksiyon; onToggle: (a: Aksiyon) => void; onDelete: (id: string) => void }) {
  ...
  return (
    <div className="flex items-start gap-3 p-3 rounded-xl transition-colors" style={{ background: "hsl(var(--admin-card-bg))", border: "1px solid hsl(var(--admin-border))", opacity: isDone ? 0.6 : 1 }}>
      <button onClick={() => onToggle(aksiyon)} className="mt-0.5 rounded border flex items-center justify-center flex-shrink-0 transition-colors" style={{ borderColor: isDone ? "#22c55e" : "hsl(var(--admin-border))", background: isDone ? "rgba(34,197,94,0.15)" : "transparent", width: 20, height: 20 }}>
        {isDone && <Check className="w-3 h-3 text-emerald-500" />}
      </button>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <Icon className="w-3.5 h-3.5 flex-shrink-0" style={{ color: turConfig.color }} />
          <span className={`text-sm font-medium truncate ${isDone ? "line-through" : ""}`} style={{ color: "hsl(var(--admin-text))" }}>{aksiyon.baslik}</span>
        ...
        <div className="flex items-center gap-2 mt-1.5">
          ...
          <span className={`text-[10px] ${isPast ? "text-red-400 font-medium" : ""}`} style={!isPast ? { color: "hsl(var(--admin-muted))" } : undefined}>{format(new Date(aksiyon.tarih), "dd MMM HH:mm", { locale: tr })}</span>
        </div>
      </div>
