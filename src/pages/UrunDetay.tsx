...
export default function UrunDetay() {
  ...
  const fetchUrun = useCallback(async () => {
    ...
    if (idsToResolve.length > 0) {
      ...
      const resolved: Record<string, string> = {};
      Object.entries(teknikData).forEach(([key, val]) => {
        if (Array.isArray(val)) resolved[key] = val.join(", ");
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 mb-8">
          ...
          <div className="lg:col-span-3 space-y-6 order-2 lg:order-1">
            ...
            {allImages.length > 1 && (
              <div className="flex sm:flex-col gap-2 shrink-0 overflow-x-auto sm:overflow-x-hidden sm:overflow-y-auto sm:max-h-[70vh] sm:min-h-0 sm:pr-1">
                {allImages.map((img, i) => (
                  <button
                    key={i}
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {benzerUrunler.map(b => {
                ...
                return (
                  <Card key={b.id} className="overflow-hidden hover:shadow-lg transition-shadow group flex flex-col cursor-pointer" onClick={() => navigate(`/urun/${b.slug || b.id}`)}>
                    ...
                    <div className="p-3 flex flex-col flex-1">
                      ...
                      {currentUserId && currentUserId !== "public" && packageInfo.paketSlug !== "ucretsiz" && (
