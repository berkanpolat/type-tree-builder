...
function LandingHeader({ logoImg: logoSrc }: { logoImg: string }) {
...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-background font-sans">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[3fr_1fr] gap-8 lg:gap-12 items-center">
            ...
            <div className="hidden lg:flex items-center justify-center">
              <img
                ...
                className="w-[28rem] h-[28rem] object-contain animate-logo-pulse"
              />
            </div>
          </div>
        </div>
      ...
      {/* ISO Banner */}
      <section className="bg-background py-8 md:py-10 border-y border-border overflow-hidden">
        <div className="max-w-5xl mx-auto px-4 md:px-6">
          <div className="grid grid-cols-4 md:flex md:items-center md:justify-center gap-4 md:gap-12">
            {isoBadges.map((badge) => (
              <div key={badge.label} className="flex items-center justify-center">
                <img
                  src={badge.img}
                  alt={badge.label}
                  loading="lazy"
                  className="w-14 h-14 md:w-[72px] md:h-[72px] object-contain opacity-60 hover:opacity-100 transition-opacity"
                />
              </div>
            ))}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-3xl mx-auto">
            ...
            >
              ...
              <div className="mb-1 flex items-baseline gap-2">
                {billingYearly ? (
                  <>
                    <span className="text-sm text-destructive line-through">{PRO_FIYATLAR.aylik.fiyat * 12}{PRO_FIYATLAR.paraBirimi}</span>
                    <span className="text-4xl font-extrabold text-foreground">{PRO_FIYATLAR.yillik.fiyat}{PRO_FIYATLAR.paraBirimi}</span>
                    <span className="text-muted-foreground text-sm">/ Yıl</span>
                  </>
                ) : (
                  <>
                    <span className="text-4xl font-extrabold text-foreground">{PRO_FIYATLAR.aylik.fiyat}{PRO_FIYATLAR.paraBirimi}</span>
                    <span className="text-muted-foreground text-sm">/ Ay</span>
                  </>
                )}
              </div>
              <p className="text-xs text-muted-foreground mb-5">Fiyatlara ödeme sırasında %{PRO_FIYATLAR.kdvOrani} KDV eklenir.</p>
              <div className="border-t border-border pt-4 space-y-3 flex-1">
                {[
