    ...
    <div className="flex items-start gap-2.5 p-2.5 rounded-lg transition-colors" style={{ background: "hsl(var(--admin-hover))", opacity: isDone ? 0.6 : 1 }}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          <span className={`text-xs font-medium truncate ${isDone ? "line-through" : ""}`} style={{ color: "hsl(var(--admin-text))" }}>{aksiyon.baslik}</span>
        </div>
        {aksiyon.aciklama && <p className="text-[11px] mt-0.5 line-clamp-2" style={{ color: "hsl(var(--admin-muted))" }}>{aksiyon.aciklama}</p>}
        <div className="flex items-center gap-2 mt-1">
          <Badge className="text-[9px] px-1 py-0" style={{ background: `${turConfig.color}20`, color: turConfig.color, borderColor: `${turConfig.color}40` }}>{turConfig.label}</Badge>
          <span className={`text-[10px] ${isPast ? "text-red-400" : ""}`} style={!isPast ? { color: "hsl(var(--admin-muted))" } : undefined}>
